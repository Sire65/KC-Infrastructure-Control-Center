...
export const RUNTIME_ATTESTATION_RULES=Object.freeze([
  ...
  'Nur frische, autoritativ transportierte Attestationen können RUNTIME_VERIFIED werden.'
]);
...
export function validateRuntimeAttestation(input,now=Date.now()){
  ...
  return {ok:issues.length===0,status:issues.length?'INVALID':'VALID',fresh,ageMs,issues};
}
...
export function evaluateStorageProtection(attestation){
  ...
  if(enc==='AES-256-GCM'&&engine==='INDEXEDDB')return{status:'SECURE',reason:'IndexedDB + AES-256-GCM durch frische Runtime-Attestation gemeldet'};
