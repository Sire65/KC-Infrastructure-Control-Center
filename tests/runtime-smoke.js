...
export async function runRuntimeSmoke(){
 ...
 out.push(check('BOOT_FRITZ_RADIO_DETAILS',Boolean(globalThis.KICC_FRITZ_RADIO_DETAILS),'FRITZ!Box Funk/Bridge-Nachweis geladen'));
 ...
 out.push(check('BOOT_INTERNET_HEALTH_SUMMARY',Boolean(globalThis.KICC_INTERNET_HEALTH_SUMMARY),'Internet-Gesamttest geladen'));
 ...
 const staleNet={measuredAt:new Date(Date.now()-30000).toISOString(),internet:{status:'OK',latencyMs:12,dnsMs:5,httpMs:20,ipv4:{status:'OK'},ipv6:{status:'OK'}},router:{status:'OK',reachable:true},wan:{status:'OK'}};
 const ih=globalThis.KICC_INTERNET_HEALTH_SUMMARY?.verdict?.(staleNet);out.push(check('INTERNET_STALE_BECOMES_UNKNOWN',ih?.result==='UNKNOWN'&&ih?.known===0,`30s alte grüne Messung → ${ih?.result||'—'} · bekannt=${ih?.known??'—'}`));
 const fr=globalThis.KICC_FRITZ_RADIO_DETAILS;out.push(check('FRITZ_STALE_REJECTED',fr?.fresh?fr.fresh(staleNet)===false:false,'FRITZ Funk-Nachweis verwirft >25s alte Messung'));
 const futureNet={...staleNet,measuredAt:new Date(Date.now()+16000).toISOString()};out.push(check('FRITZ_FUTURE_REJECTED',fr?.fresh?fr.fresh(futureNet)===false:false,'FRITZ Funk-Nachweis verwirft Zukunft >15s'));
 ...
 const mirror=globalThis.KICC_MIRROR?.flow;if(mirror){const mh=globalThis.KICC_MIRROR.health?.();out.push(check('MIRROR_FAILOVER_EVIDENCE',mh?.readyForFailover!==true||mirror.trust==='OBSERVED_REMOTE',`Failover-ready=${mh?.readyForFailover?'JA':'NEIN'} · Trust=${mirror.trust}`));out.push(check('MIRROR_CURRENT_EVIDENCE_FIELDS','currentMismatchCount' in mirror&&'currentConflictCount' in mirror,`aktuelle Abweichungen=${mirror.currentMismatchCount??'—'} · Konflikte=${mirror.currentConflictCount??'—'}`));const ctx=kicc?.failoverContext||{};out.push(check('MIRROR_FAILOVER_USES_CURRENT',ctx.mirrorMismatchCount==null||ctx.mirrorMismatchCount===mirror.currentMismatchCount,`Failover-Abweichung aktuell=${ctx.mirrorMismatchCount??'—'} · 24h=${mirror.mismatchCount24h??'—'}`));}else out.push(check('BOOT_MIRROR',false,'Mirror-Runtime nicht geladen'));
