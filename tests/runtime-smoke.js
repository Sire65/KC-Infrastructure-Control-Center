...
export async function runRuntimeSmoke(){
 ...
 const unknownProgramStatus=globalThis.KICC_DASHBOARD_INSTRUMENTS?.programStatus?.({id:'__kicc_tuev_missing_program__'});out.push(check('PROGRAM_NO_EVIDENCE_UNKNOWN',unknownProgramStatus==='UNKNOWN',`Programm ohne Heartbeat → ${unknownProgramStatus||'—'}`));
 ...
 const internetApi=globalThis.KICC_INTERNET_INSTRUMENTS;out.push(check('INTERNET_INSTRUMENT_API',Boolean(internetApi?.render&&internetApi?.mount),'Internet-Instrumente besitzen Render/Mount API'));
 ...
 const reportEl=document.getElementById('mirrorMorningReport'),reportBody=reportEl?.querySelector('[data-mmr-body]')?.textContent||'';out.push(check('MIRROR_REPORT_VISIBLE_STATE',Boolean(reportEl&&reportBody.trim()),reportBody.trim().slice(0,140)||'Morgenreport ohne sichtbaren Zustand'));
