...
const migrations=[
  ...
  createRegionMigration({id:'mig-b2-pc-eu',resourceId:'storage-b2-pc',provider:'Backblaze B2',sourceRegion:'US · noch zu verifizieren',targetRegion:'eu-central · Amsterdam',targetJurisdiction:'EU'})
];
...
function render(){
  ...
  if(!host)return;
  const planned=migrations.map(m=>`<article class="db-card"><div class="db-title"><span class="status-chip ${badgeClass(m.state)}"><span class="dot"></span>${m.state}</span><strong>${m.provider}</strong></div><div class="db-meta"><span>Quelle: ${m.sourceRegion}</span><span>Ziel: ${m.targetRegion}</span><span>Ziel-Rechtsraum: ${m.targetJurisdiction}</span><span>Ressource: ${m.resourceId}</span></div><div class="caps"><span class="cap unknown">Recovery Point</span><span class="cap unknown">Kopie</span><span class="cap unknown">Delta-Sync</span><span class="cap unknown">Verifikation</span><span class="cap unknown">Cutover</span><span class="cap unknown">Rollback</span></div><div class="db-note">Alter Standort wird niemals automatisch gelöscht. Umschaltung erst nach Integritätsprüfung und expliziter Freigabe.</div></article>`).join('');
