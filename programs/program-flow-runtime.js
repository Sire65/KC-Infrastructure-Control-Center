...
function purge(){const min=Date.now()-PROGRAM_FLOW_MAX_AGE_MS;while(events.length&&new Date(events[0].measuredAt).getTime()<min)events.shift();if(events.length>MAX_EVENTS)events.splice(0,events.length-MAX_EVENTS);}
function ingest(raw){try{if(raw?.schema&&raw.schema!==PROGRAM_FLOW_SCHEMA)return false;const flow=normalizeProgramFlow(raw);events.push(flow);purge();globalThis.dispatchEvent(new CustomEvent('kicc:program-flow-ingested',{detail:flow}));return flow;}catch{return false;}}
function bind(){
