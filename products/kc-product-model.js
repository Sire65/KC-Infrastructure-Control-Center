...
export function makeKcProduct({id,name,repo,kind,owner='Sire65',critical=false,healthPolicy=PRODUCT_HEALTH_POLICY.RUNTIME_REQUIRED,dependencies={}}){
  return {
    ...
    repoHealth:'UNKNOWN',repoMeasuredAt:null,repoTrust:'UNVERIFIED',
...
export function evaluateProductHealth(product,now=Date.now()){
  ...
  if(!product.deploymentUrl&&product.telemetryHealth==='UNKNOWN')return{status:'UNKNOWN',reason:'Repository kann erreichbar sein, aber Runtime-Nachweis fehlt'};
