...
function mslLabel(msl){return msl?.recovery?.active?`${msl.label} · ${msl.recovery.clearCount}/${msl.recovery.clearRequired}`:msl?.label||'—';}
...
#actions .kicc-action-mirror-meta{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:6px;width:100%;margin-top:2px}
...
#actions .kicc-msl-recommendation{font-size:10px;font-weight:700;color:#e2e8f0;text-align:center;margin-top:-1px}
@media(max-width:1200px){#actions .kicc-action-mirror-meta{grid-template-columns:repeat(3,1fr)}}
...
function mirrorDetails(){const host=document.querySelector('#topology .flow-state');const flow=globalThis.KICC_MIRROR?.flow;if(!host||!flow)return;const msl=globalThis.KICC_MIRROR?.msl?.();let box=host.querySelector('.kicc-mirror-mini');if(!box){box=document.createElement('div');box.className='kicc-mirror-mini';host.appendChild(box);}box.innerHTML=`<div><small>Sync-Lag</small><strong>${fmtSec(flow.syncLagSec)}</strong></div><div><small>Letzter Erfolg</small><strong>${fmtAge(flow.lastSuccessAt)}</strong></div><div><small>Fehlerquote 24h</small><strong>${fmtRate(flow)}</strong></div><div><small>MSL</small><strong>${msl?`${mslLabel(msl)} · ${msl.trend}`:'—'}</strong></div>`;}
function polishActions(){const stack=document.querySelector('#actions .action-stack');const flow=globalThis.KICC_MIRROR?.flow;const state=globalThis.KICC?.currentFailoverState?.();const msl=globalThis.KICC_MIRROR?.msl?.();if(!stack||!flow||!state)return;const head=stack.querySelector(':scope > strong');if(state.state==='DEGRADED'&&state.primary==='SUPABASE'&&Number.isFinite(flow.syncLagSec)&&flow.syncLagSec>0){if(head)head.textContent=msl?`MIRROR ${mslLabel(msl).toUpperCase()}`:'MIRROR VERZÖGERT';let meta=stack.querySelector('.kicc-action-mirror-meta');if(!meta){meta=document.createElement('div');meta.className='kicc-action-mirror-meta';const firstAction=stack.querySelector('.action-item');if(firstAction)firstAction.before(meta);else stack.appendChild(meta);}meta.innerHTML=`<div><small>Sync-Lag</small><strong>${fmtSec(flow.syncLagSec)}</strong></div><div><small>Ø 1h</small><strong>${fmtSec(msl?.avg1h)}</strong></div><div><small>Max 1h</small><strong>${fmtSec(msl?.max1h)}</strong></div><div><small>Trend</small><strong>${msl?.trend||'—'}</strong></div><div><small>Letzter Erfolg</small><strong>${fmtAge(flow.lastSuccessAt)}</strong></div><div><small>Fehlerquote 24h</small><strong>${fmtRate(flow)}</strong></div>`;let rec=stack.querySelector('.kicc-msl-recommendation');if(!rec){rec=document.createElement('div');rec.className='kicc-msl-recommendation';meta.after(rec);}rec.textContent=msl?.recommendation||'Mirror beobachten';}else{stack.querySelector('.kicc-action-mirror-meta')?.remove();stack.querySelector('.kicc-msl-recommendation')?.remove();}}
