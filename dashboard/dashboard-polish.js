...
function mirrorDetails(){const host=document.querySelector('#topology .flow-state');const flow=globalThis.KICC_MIRROR?.flow;if(!host||!flow)return;const msl=globalThis.KICC_MIRROR?.msl?.();let box=host.querySelector('.kicc-mirror-mini');if(!box){box=document.createElement('div');box.className='kicc-mirror-mini';host.appendChild(box);}box.innerHTML=`<div><small>Sync-Lag</small><strong>${Number.isFinite(flow.syncLagSec)?`${flow.syncLagSec}s`:'—'}</strong></div><div><small>Letzter Erfolg</small><strong>${fmtAge(flow.lastSuccessAt)}</strong></div><div><small>Fehlerquote 24h</small><strong>${fmtRate(flow)}</strong></div><div><small>MSL</small><strong>${msl?`${msl.label} · ${msl.trend}`:'—'}</strong></div>`;}
function polishActions(){const stack=document.querySelector('#actions .action-stack');const flow=globalThis.KICC_MIRROR?.flow;const state=globalThis.KICC?.currentFailoverState?.();const msl=globalThis.KICC_MIRROR?.msl?.();if(!stack||!flow||!state)return;const head=stack.querySelector(':scope > strong');if(state.state==='DEGRADED'&&state.primary==='SUPABASE'&&Number.isFinite(flow.syncLagSec)&&flow.syncLagSec>0){if(head)head.textContent=msl?`MIRROR ${msl.label.toUpperCase()}`:'MIRROR VERZÖGERT';let meta=stack.querySelector('.kicc-action-mirror-meta');if(!meta){meta=document.createElement('div');meta.className='kicc-action-mirror-meta';const firstAction=stack.querySelector('.action-item');if(firstAction)firstAction.before(meta);else stack.appendChild(meta);}meta.innerHTML=`<div><small>Sync-Lag</small><strong>${flow.syncLagSec}s</strong></div><div><small>Ø 1h</small><strong>${fmtSec(msl?.avg1h)}</strong></div><div><small>Max 1h</small><strong>${fmtSec(msl?.max1h)}</strong></div><div><small>Trend</small><strong>${msl?.trend||'—'}</strong></div><div><small>Letzter Erfolg</small><strong>${fmtAge(flow.lastSuccessAt)}</strong></div><div><small>Fehlerquote 24h</small><strong>${fmtRate(flow)}</strong></div>`;let rec=stack.querySelector('.kicc-msl-recommendation');if(!rec){rec=document.createElement('div');rec.className='kicc-msl-recommendation';meta.after(rec);}rec.textContent=msl?.recommendation||'Mirror beobachten';}else{stack.querySelector('.kicc-action-mirror-meta')?.remove();stack.querySelector('.kicc-msl-recommendation')?.remove();}}
