...
import json,math,os,platform,re,socket,ssl,statistics,subprocess,threading,time,urllib.parse,urllib.request,xml.etree.ElementTree as ET
...
PROBE_TARGETS=[('Cloudflare','1.1.1.1'),('Google','8.8.8.8'),('Quad9','9.9.9.9')];_lock=threading.Lock();_snapshot={};_speed={'status':'IDLE'};_bufferbloat={'status':'IDLE'};_history=[]
...
def _run_ping(host,count=4,timeout_ms=1200,ipv6=False):
 system=platform.system().lower();cmd=['ping','-n',str(count),'-w',str(timeout_ms)]+(['-6'] if ipv6 else [])+[host] if system=='windows' else ['ping']+(['-6'] if ipv6 else [])+['-c',str(count),'-W',str(max(1,math.ceil(timeout_ms/1000))),host]
 ...
 loss=max(0,min(100,(count-len(vals))*100/max(1,count)));return {'host':host,'ok':bool(vals),'latencyMs':round(statistics.median(vals),2) if vals else None,'jitterMs':round(statistics.pstdev(vals),2) if len(vals)>1 else (0.0 if vals else None),'packetLossPct':round(loss,2),'samplesMs':vals}
...
def _dns_probe():
 t=time.perf_counter()
 try:socket.getaddrinfo('example.com',443,type=socket.SOCK_STREAM);return {'ok':True,'latencyMs':round((time.perf_counter()-t)*1000,2)}
...
def _https_probe():
 ...
 try:
  ...
  with ctx.wrap_socket(raw,server_hostname=host) as s:tls=(time.perf_counter()-tt)*1000;s.sendall(b'HEAD / HTTP/1.1\r\nHost: www.cloudflare.com\r\nConnection: close\r\n\r\n');s.settimeout(3);data=s.recv(256)
  return {'ok':bool(data),'tcpMs':round(tcp,2),'tlsMs':round(tls,2),'latencyMs':round((time.perf_counter()-t)*1000,2)}
...
def _soap(service,control,action):
 url=f'http://{FRITZ_HOST}:49000{control}';body=f'''<?xml version="1.0"?><s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"><s:Body><u:{action} xmlns:u="{service}"></u:{action}></s:Body></s:Envelope>'''.encode();req=urllib.request.Request(url,data=body,method='POST',headers={'Content-Type':'text/xml; charset="utf-8"','SOAPACTION':f'"{service}#{action}"'})
 try:
...
def _fritz_probe():
 ...
 if dev:r.update(model=dev.get('ModelName') or r['model'],firmware=dev.get('SoftwareVersion'))
 connected=str(wi.get('ConnectionStatus','')).lower()=='connected' if wi else None
 def rate(d,k,div):v=str(d.get(k,'') if d else '');return round(int(v)/div,3) if v.isdigit() else None
...
def _speed_worker(profile):
 ...
 try:
  ...
  with _lock:_speed={'status':'DONE','profile':profile,'measuredAt':utcnow(),'bytes':received,'seconds':round(sec,2),'downloadMbps':round(received*8/sec/1e6,2)}
 except Exception as e:
  with _lock:_speed={'status':'FAILED','profile':profile,'measuredAt':utcnow(),'error':str(e)}
def _bufferbloat_worker():
 global _bufferbloat
 with _lock:_bufferbloat={'status':'RUNNING','startedAt':utcnow()}
 try:
  idle=_run_ping('1.1.1.1',6,1200);load_done=threading.Event()
  def load():
   try:
    with urllib.request.urlopen('https://speed.cloudflare.com/__down?bytes=5000000',timeout=20) as x:
     while x.read(65536):pass
   finally:load_done.set()
  threading.Thread(target=load,daemon=True).start();samples=[]
  while not load_done.is_set():
   p=_run_ping('1.1.1.1',1,1200);samples+=p.get('samplesMs') or []
   if len(samples)>=12:break
  loaded=statistics.median(samples) if samples else None;base=idle.get('latencyMs');delta=(loaded-base) if loaded is not None and base is not None else None;grade='A' if delta is not None and delta<15 else 'B' if delta is not None and delta<30 else 'C' if delta is not None and delta<60 else 'D' if delta is not None else 'UNKNOWN'
  with _lock:_bufferbloat={'status':'DONE','measuredAt':utcnow(),'idleLatencyMs':base,'loadedLatencyMs':round(loaded,2) if loaded is not None else None,'increaseMs':round(delta,2) if delta is not None else None,'grade':grade,'samples':len(samples)}
 except Exception as e:
  with _lock:_bufferbloat={'status':'FAILED','measuredAt':utcnow(),'error':str(e)}
def worker():
 global _snapshot
 while True:
  try:s=collect_snapshot()
  except Exception as e:s={'schema':'kicc.local-network.v3','status':'FAILED','measuredAt':utcnow(),'error':str(e)}
  with _lock:
   _snapshot=s;_history.append({'measuredAt':s.get('measuredAt'),'status':s.get('status'),'latencyMs':s.get('internet',{}).get('latencyMs'),'jitterMs':s.get('internet',{}).get('jitterMs'),'packetLossPct':s.get('internet',{}).get('packetLossPct')});cut=time.time()-86400;_history[:]=[x for x in _history if datetime.fromisoformat(x['measuredAt']).timestamp()>=cut]
  time.sleep(10)
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if u.path=='/health':
   with _lock:d={'ok':True,'service':'kc-local-agent','measuredAt':_snapshot.get('measuredAt')}
  elif u.path=='/v1/network':
   with _lock:d=dict(_snapshot)
  elif u.path=='/v1/history':
   q=urllib.parse.parse_qs(u.query);minutes=max(1,min(1440,int(q.get('minutes',['60'])[0])));cut=time.time()-minutes*60
   with _lock:d={'minutes':minutes,'items':[x for x in _history if datetime.fromisoformat(x['measuredAt']).timestamp()>=cut]}
  else:self._headers(404);self.wfile.write(b'{"error":"not found"}');return
  self._headers();self.wfile.write(json.dumps(d,ensure_ascii=False).encode())
 def do_POST(self):
  u=urllib.parse.urlparse(self.path)
  if u.path not in ('/v1/speedtest','/v1/bufferbloat'):self._headers(404);self.wfile.write(b'{"error":"not found"}');return
  if u.path=='/v1/speedtest':
   ...
   threading.Thread(target=_speed_worker,args=(profile,),daemon=True).start();resp={'accepted':True,'profile':profile}
  else:
   with _lock:busy=_bufferbloat.get('status')=='RUNNING'
   if busy:self._headers(409);self.wfile.write(b'{"error":"bufferbloat already running"}');return
   threading.Thread(target=_bufferbloat_worker,daemon=True).start();resp={'accepted':True}
  self._headers(202);self.wfile.write(json.dumps(resp).encode())
 def log_message(self,*a):return
def main():threading.Thread(target=worker,daemon=True).start();server=ThreadingHTTPServer((HOST,PORT),Handler);print(f'KC Local Agent 0.3.0 auf http://{HOST}:{PORT}');server.serve_forever()
