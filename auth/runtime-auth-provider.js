...
export function setSupabaseSession(projectId,session){
  const normalized=normalizeSession(session);
  if(!normalized)throw new Error('Valid short-lived Supabase access token required');
  sessions.set(String(projectId),normalized);
  return true;
}
export function clearSupabaseSession(projectId=null){
  if(projectId)sessions.delete(String(projectId));else sessions.clear();
}
export function registerSupabaseSessionResolver(projectId,resolver){
  if(typeof resolver!=='function')throw new TypeError('resolver must be a function');
  resolvers.set(String(projectId),resolver);
}
async function resolveSession(projectId){
  let current=normalizeSession(sessions.get(projectId));
  if(current)return current;
  sessions.delete(projectId);
  const resolver=resolvers.get(projectId);
  if(!resolver)return null;
  current=normalizeSession(await resolver());
...
export function status(){
  return Object.entries(PROJECT_BY_TARGET).map(([targetId,projectId])=>{
    ...
    return{targetId,projectId,state:s?'READY':'AUTH_REQUIRED',expiresAt:s?.expiresAt||null,storage:'MEMORY_ONLY'};
  });
}

// Tokens are intentionally memory-only. No localStorage, IndexedDB, service worker or repository persistence.
globalThis.KICC_AUTH={
  ...(globalThis.KICC_AUTH||{}),
  setSupabaseSession,clearSupabaseSession,registerSupabaseSessionResolver,getSupabaseBridgeAuth,getNeonBridgeAuth,status
};
